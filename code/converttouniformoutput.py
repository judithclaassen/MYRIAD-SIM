# -*- coding: utf-8 -*-
"""
Created on Wed Mar  5 11:04:57 2025

@author: jcl202
"""
#code to convert uniform model output with the variable marginal distribution back tot he original data space 
...
for namei in names:
    ...
    for j in list(range(1,13)):
        ...
        for i in range(len(ys)):
            ...
            img2[np.where(months == np.int64(j)), ys[i], xs[i]] =  dsx.ppf(img[np.where(months == np.int64(j)), ys[i], xs[i]], *paramx )

    trial.u.values = img2.copy()
    trial.to_netcdf(filepath + namei +'converted.nc')